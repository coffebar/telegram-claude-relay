...
class ConversationMonitor:
    ...
    async def process_transcript(self, transcript_path: str, session_id: str) -> None:
        ...
        logger.info("Processing transcript", path=transcript_path, session_id=session_id)
        try:
            ...
            logger.info("Parsed messages from transcript", 
            ...
            if current_turn_messages:
                ...
                logger.info("No messages from current turn to relay", session_id=session_id)
                
        except Exception as e:
            logger.error("Error processing transcript", error=str(e), path=transcript_path)
    
    async def _parse_all_messages(self, path: Path, session_id: str) -> List[Dict[str, Any]]:
    ...
    def _extract_current_turn(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _extract_message_data(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        logger.debug("Extracting message", msg_type=msg_type, available_keys=list(data.keys()))
        ...
        if msg_type == "user" and isinstance(message.get("content"), list):
            ...
            if content_list and isinstance(content_list[0], dict) and content_list[0].get("type") == "tool_result":
        ...
        logger.debug("Extracted message", role=result["role"], has_content=bool(content), tool_count=len(tool_calls))
        return result
    
    async def _relay_to_telegram(self, messages: List[Dict[str, Any]], session_id: str) -> None:
    ...
    def _format_hook_notification(self, notification: Dict[str, Any]) -> Optional[str]:
        ...
        if notif_type == "user_prompt":
            ...
            tool_name = notification.get("tool_name", "Unknown")
            result_preview = notification.get("result_preview")
            ...
            if tool_name == "Edit":
                ...
                return f"✅ **File search completed**"
            elif tool_name == "MultiEdit":
                return f"✅ **Multi-edit completed**"
            else:
