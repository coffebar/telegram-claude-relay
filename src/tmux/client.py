...
class TmuxClient:
    ...
    async def discover_claude_pane() -> str:
        ...
        try:
            ...
            raise TmuxCommandError("tmux command not found. Is tmux installed?")
        except Exception as e:
            raise TmuxCommandError(f"Failed to discover claude pane: {e}")
    ...
    async def _run_tmux_command(self, args: List[str]) -> str:
        ...
        try:
            ...
            raise TmuxCommandError(f"Failed to execute tmux command: {e}")
    ...
    async def send_command(self, text: str) -> None:
        ...
        await self._run_tmux_command([
            "send-keys", "-t", self.pane_target, text
        ])
        ...
        await self._run_tmux_command([
            "send-keys", "-t", self.pane_target, "Enter"
        ])
    ...
    async def capture_output(self, lines: int = 100) -> str:
        ...
        try:
            return await self._run_tmux_command([
                "capture-pane", "-t", self.pane_target,
                "-S", f"-{lines}", "-p"
            ])
        except TmuxCommandError as e:
            if "can't find pane" in str(e).lower():
                raise TmuxPaneNotFoundError(f"Pane not found: {self.pane_target}")
            raise
    ...
    async def get_pane_info(self) -> Dict[str, str]:
        ...
        try:
            info = await self._run_tmux_command([
                "display-message", "-t", self.pane_target, "-p",
                "#{session_name}:#{window_index}.#{pane_index} #{pane_current_command}"
            ])
            return {"pane": info}
        except TmuxCommandError as e:
    ...
    async def wait_for_output_change(self, initial_output: str, timeout: int = 30, poll_interval: float = 1.0) -> str:
        ...
        raise TmuxResponseTimeoutError(f"No output change detected within {timeout} seconds")
